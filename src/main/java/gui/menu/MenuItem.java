...
public class MenuItem extends JPanel {
    ...
    private Icon getIcon(int option) {
        ...
        if (option == 1) {
            icon = new FlatSVGIcon(getClass().getResource("/img/icon/svg/" + menuIndex + ".svg"));
        } else if (option == 2) {
            icon = new FlatSVGIcon(getClass().getResource("/img/icon/svg/manager/" + menuIndex + ".svg"));
        }

        FlatSVGIcon.ColorFilter colorFilter = new FlatSVGIcon.ColorFilter();

        colorFilter.add(Color.decode("#000000"), Color.white);
    ...
    private void init(int option) throws RemoteException {
        ...
        for (int i = 0; i < menus.length; i++) {
            ...
            if (i == 0) {
                menuItem.setIcon(getIcon(option));
